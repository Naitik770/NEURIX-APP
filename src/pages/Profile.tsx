...
export default function Profile() {
  ...
  const AVATAR_OPTIONS = [
    ...
    'Aria', 'Blake', 'Cody', 'Dante', 'Eden', 'Faith', 'Gage', 'Hope',
    'Iris', 'Jude', 'Kai', 'Lane', 'Mina', 'Noah', 'Owen', 'Paige',
    'Quincy', 'Reed', 'Sage', 'Tessa', 'Uri', 'Violet', 'Wyatt', 'Xavier',
    'Yasmine', 'Zane', 'Axel', 'Brooke', 'Caleb', 'Demi', 'Ezra', 'Fiona',
    'Gavin', 'Hazel', 'Ian', 'Jade', 'Kira', 'Liam', 'Maya', 'Nico'
  ];

  useEffect(() => {
    if (profile) {
      setAvatarSeed(profile.avatarSeed || user?.uid || '');
    }
  }, [profile, user]);
    ...
    <div className="p-6 pt-12 min-h-screen bg-[#FDFBF7] dark:bg-gray-900 pb-32 transition-colors duration-300">
      ...
      {showAvatarModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-6">
          <div className="bg-white dark:bg-gray-800 rounded-3xl p-6 max-w-sm w-full shadow-2xl flex flex-col max-h-[80vh] transition-colors duration-300">
            <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-4">Choose Avatar</h3>
            <div className="grid grid-cols-4 gap-4 mb-6 overflow-y-auto pr-2 custom-scrollbar">
              {AVATAR_OPTIONS.map((seed) => (
                <button 
                  key={seed}
                  onClick={() => setAvatarSeed(seed)}
                  className={`w-16 h-16 rounded-full overflow-hidden border-2 transition-all ${avatarSeed === seed ? 'border-orange-500 scale-110' : 'border-transparent hover:border-orange-200 dark:hover:border-orange-500/50'}`}
                >
            ...
            <div className="flex gap-4 pt-4 border-t border-gray-100 dark:border-gray-700 transition-colors duration-300">
              ...
              <button onClick={() => { handleSaveAvatar(); setShowAvatarModal(false); }} className="flex-1 py-3 bg-orange-500 text-white rounded-xl font-semibold hover:bg-orange-600 transition-colors">Save</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 text-center shadow-sm transition-colors duration-300">
          ...
          <p className="text-xl font-bold text-gray-900 dark:text-white">{profile?.lifeScore || 50}</p>
