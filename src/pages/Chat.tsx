...
export default function Chat() {
  ...
  return (
    <div className="flex flex-col h-full bg-[#FDFBF7] dark:bg-gray-900 overflow-hidden relative">
      ...
      {/* Messages Area */}
      <div className="flex-1 overflow-y-auto overflow-x-hidden overscroll-x-none p-4 custom-scrollbar bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] bg-fixed opacity-90 pb-32">
        {messages.length === 0 ? (
      ...
      <AnimatePresence>
        {(attachment || replyingTo) && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute bottom-[90px] left-4 right-4 bg-white/90 dark:bg-gray-800/90 backdrop-blur-md p-3 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 z-30 flex items-center gap-3">
      ...
      {/* Input Area - REFINED INSTAGRAM STYLE */}
      <div className="absolute bottom-0 left-0 right-0 w-full flex justify-center items-end p-4 pb-6 bg-gradient-to-t from-white/40 dark:from-gray-900/40 via-white/20 dark:via-gray-900/20 to-transparent backdrop-blur-[6px] z-40">
        {editingMessage ? (
          <form onSubmit={handleEditMessage} className="flex flex-col gap-2 w-full max-w-3xl bg-white dark:bg-gray-800 p-3 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700">
            ...
            <div className="flex items-center gap-2">
              <input 
                ...
                className="flex-1 bg-gray-50 dark:bg-gray-900 border-none rounded-xl px-4 py-2.5 outline-none text-sm text-gray-900 dark:text-white"
                autoFocus 
              />
          ...
          <form onSubmit={handleSendMessage} className="flex items-center gap-2 w-full max-w-3xl">
            <input type="file" ref={fileInputRef} onChange={handleFileSelect} className="hidden" accept="image/*,video/*,.pdf,.doc,.docx,.txt" />
            
            <div className="flex-1 flex items-center bg-white dark:bg-gray-900/90 rounded-full pl-2 pr-1 py-1 shadow-lg shadow-gray-200/50 dark:shadow-none border border-gray-100 dark:border-gray-700 transition-all">
              <button 
                type="button" 
                onClick={() => fileInputRef.current?.click()} 
                className="p-2.5 text-gray-400 hover:text-orange-500 transition-colors shrink-0"
              >
              ...
              <button 
                ...
                className={`p-2.5 transition-colors shrink-0 ${isListening ? 'text-orange-500 animate-pulse' : 'text-gray-400 hover:text-orange-500'}`}
              >
              ...
              <button 
                ...
                className={`w-10 h-10 rounded-full flex items-center justify-center transition-all shrink-0 ml-1 ${
                  (newMessage.trim() || attachment) 
                    ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/30 scale-100 active:scale-90' 
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="relative w-full max-w-sm bg-white dark:bg-gray-800 rounded-3xl p-6 shadow-2xl">
              ...
              <div className="flex gap-2 mb-4">
                <button onClick={() => {setNicknameTarget('friend'); setTempNickname(nicknames[friendId!] || '');}} className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-colors ${nicknameTarget === 'friend' ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/20' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300'}`}>Friend</button>
                <button onClick={() => {setNicknameTarget('me'); setTempNickname(nicknames[user?.uid!] || '');}} className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-colors ${nicknameTarget === 'me' ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/20' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300'}`}>Me</button>
              </div>
              <input 
                type="text" value={tempNickname} onChange={(e) => setTempNickname(e.target.value)} 
                className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-700 rounded-2xl px-4 py-4 mb-6 outline-none focus:ring-2 focus:ring-orange-500/20 dark:text-white" 
                placeholder="Enter nickname..." 
              />
              <div className="flex gap-3">
                <button onClick={() => setShowNicknameModal(false)} className="flex-1 py-4 bg-gray-100 dark:bg-gray-700 rounded-2xl font-bold text-gray-600 dark:text-gray-300 transition-colors">Cancel</button>
