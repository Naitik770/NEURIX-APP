...
export default function Chat() {
  ...
  return (
    <div className="flex flex-col h-full bg-[#FDFBF7] dark:bg-gray-900 overflow-hidden relative">
      ...
      {/* Messages Area - FIXED OVERFLOW AND SWIPE LOGIC */}
      <div className="flex-1 overflow-y-auto overflow-x-hidden overscroll-x-none p-4 custom-scrollbar bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] bg-fixed opacity-90">
        {messages.length === 0 ? (
      ...
      <AnimatePresence>
        {(attachment || replyingTo) && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute bottom-[88px] left-4 right-4 bg-white dark:bg-gray-800 p-3 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 z-30 flex items-center gap-3">
             <div className="flex-1 min-w-0">
               <p className="text-xs font-bold text-orange-500 mb-0.5">{attachment ? 'Attachment' : `Replying to ${replyingTo.senderName}`}</p>
               <p className="text-sm text-gray-600 dark:text-gray-300 truncate">{attachment ? attachment.name : replyingTo.text}</p>
             </div>
             <button onClick={() => {setAttachment(null); setReplyingTo(null)}} className="p-2 bg-gray-100 dark:bg-gray-700 rounded-full"><X className="w-4 h-4"/></button>
          </motion.div>
        )}
      </AnimatePresence>

      {/* Input Area - FIXED ALIGNMENT AND CENTERING */}
      <div className="flex-shrink-0 p-4 bg-white/50 dark:bg-gray-900/50 backdrop-blur-sm z-20 border-t border-gray-100 dark:border-gray-800">
        {editingMessage ? (
          <form onSubmit={handleEditMessage} className="flex flex-col gap-2 max-w-4xl mx-auto bg-white dark:bg-gray-800 p-3 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700">
            <div className="flex items-center justify-between px-2">
              ...
              <button type="button" onClick={() => { setEditingMessage(null); setEditMessageText(''); }} className="text-gray-400"><X className="w-4 h-4"/></button>
            </div>
            <div className="flex items-center gap-2">
              <input type="text" value={editMessageText} onChange={(e) => setEditMessageText(e.target.value)} className="flex-1 bg-gray-50 dark:bg-gray-900 border-none rounded-xl px-4 py-2.5 outline-none text-sm" autoFocus />
              <button type="submit" className="w-10 h-10 rounded-xl bg-green-500 text-white flex items-center justify-center"><Check className="w-4 h-4"/></button>
            </div>
          </form>
        ) : (
          <form onSubmit={handleSendMessage} className="flex items-center gap-2 w-full max-w-3xl mx-auto">
            ...
            <div className="flex-1 flex items-center bg-white dark:bg-gray-800 rounded-full px-3 py-1 shadow-lg shadow-gray-200/50 dark:shadow-none border border-gray-100 dark:border-gray-700 transition-all">
              <button type="button" onClick={() => fileInputRef.current?.click()} className="p-2 text-gray-400 hover:text-orange-500 shrink-0"><Paperclip className="w-5 h-5" /></button>
              <input type="text" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder="Message..." className="flex-1 bg-transparent border-none px-2 py-2.5 outline-none text-[15px] text-gray-900 dark:text-white min-w-0" />
              <button type="button" onClick={startListening} className={`p-2 shrink-0 ${isListening ? 'text-orange-500 animate-pulse' : 'text-gray-400'}`}><Mic className="w-5 h-5" /></button>
              <button type="submit" disabled={!newMessage.trim() && !attachment} className={`w-9 h-9 rounded-full flex items-center justify-center transition-all shrink-0 ml-1 ${(newMessage.trim() || attachment) ? 'bg-orange-500 text-white shadow-lg' : 'bg-gray-100 dark:bg-gray-700 text-gray-400'}`}>
      ...
      <AnimatePresence>
        {showNicknameModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="relative w-full max-w-sm bg-white dark:bg-gray-800 rounded-3xl p-6">
              <h3 className="text-xl font-bold mb-4">Set Nickname</h3>
              <div className="flex gap-2 mb-4">
                <button onClick={() => {setNicknameTarget('friend'); setTempNickname(nicknames[friendId!] || '');}} className={`flex-1 py-2 rounded-xl text-xs font-bold ${nicknameTarget === 'friend' ? 'bg-orange-500 text-white' : 'bg-gray-100 dark:bg-gray-700'}`}>Friend</button>
                <button onClick={() => {setNicknameTarget('me'); setTempNickname(nicknames[user?.uid!] || '');}} className={`flex-1 py-2 rounded-xl text-xs font-bold ${nicknameTarget === 'me' ? 'bg-orange-500 text-white' : 'bg-gray-100 dark:bg-gray-700'}`}>Me</button>
              </div>
              <input type="text" value={tempNickname} onChange={(e) => setTempNickname(e.target.value)} className="w-full bg-gray-50 dark:bg-gray-900 rounded-2xl px-4 py-4 mb-6 outline-none" placeholder="Enter nickname..." />
              <div className="flex gap-3">
                <button onClick={() => setShowNicknameModal(false)} className="flex-1 py-4 bg-gray-100 dark:bg-gray-700 rounded-2xl font-bold">Cancel</button>
                <button onClick={handleSetNickname} className="flex-1 py-4 bg-orange-500 text-white rounded-2xl font-bold">Save</button>
              </div>
            </motion.div>
          </div>
        )}
      ...
      <AnimatePresence>
        {contextMenu && (
          <motion.div 
            ...
            style={{ position: 'fixed', top: Math.min(contextMenu.y, window.innerHeight - 200), left: Math.min(contextMenu.x, window.innerWidth - 180), zIndex: 110 }}
            className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-xl rounded-2xl shadow-2xl border border-gray-200/50 py-1.5 min-w-[160px]"
          >
            {contextMenu.msg.text && <button onClick={() => {navigator.clipboard.writeText(contextMenu.msg.text); toast.success('Copied'); setContextMenu(null);}} className="w-full px-4 py-2 text-left text-sm flex items-center gap-3"><Copy className="w-4 h-4" /> Copy</button>}
            <button onClick={() => { setReplyingTo(contextMenu.msg); setContextMenu(null); }} className="w-full px-4 py-2 text-left text-sm flex items-center gap-3"><CornerUpLeft className="w-4 h-4" /> Reply</button>
            {contextMenu.msg.senderId === user?.uid && (
              <>
                {contextMenu.msg.text && <button onClick={() => { setEditingMessage(contextMenu.msg); setEditMessageText(contextMenu.msg.text); setContextMenu(null); }} className="w-full px-4 py-2 text-left text-sm flex items-center gap-3"><Edit2 className="w-4 h-4" /> Edit</button>}
                <button onClick={() => { handleDeleteMessage(contextMenu.msg.id); setContextMenu(null); }} className="w-full px-4 py-2 text-left text-sm text-red-500 flex items-center gap-3"><Trash2 className="w-4 h-4" /> Unsend</button>
              </>
            )}
          </motion.div>
        )}
